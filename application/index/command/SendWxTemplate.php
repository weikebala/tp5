...
class SendWxTemplate extends Command
{
    ...
    private $token;
    private $redisName = 'weixintemplate';
    private $redis;

    protected function configure()
    {
        $this->setName('sendWx')->setDescription('Here is the weixin send template ');
    }
    ...
    private function doQuece(Output $output){
        ...
        while($this->redis->lLen($this->redisName) > 0){
            ...
            $rt = https_request('https://api.weixin.qq.com/cgi-bin/message/template/send?access_token='.$access_token,json_encode($senddata,JSON_UNESCAPED_UNICODE));
            $output->writeln(json_encode($rt));//exit;
            if($rt==false){
                $this->redis->lPush($this->redisName, $openid);
            }else{
                if($rt['errmsg'] == 'ok'){
                    Db::table('pigcms_send_message')->where('id',$this->messageid)->setInc('reachcount');
